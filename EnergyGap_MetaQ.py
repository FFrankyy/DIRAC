...
def getEnergyList(g, sol):
    ...
    for node in sol:
        ...
        states[node] *= -1
        energy += delta
        print ('Lattice_dim: %d, test_scale: %d, graph: %d, MetaQ energy: %.4f'%(args.lattice_dim, args.test_scale, args.gid, energy/len(g)))
        energy_list.append(energy/len(g))
    return energy_list
...
def Greedy(g, states):
    ...
    while not stopCondition:
        ...
        if best_delta > 0.0:
        	...
        	print ('Lattice_dim: %d, test_scale: %d, graph: %d, Greedy energy: %.4f'%(args.lattice_dim, args.test_scale, args.gid, energy/len(g)))
        	energy_list.append(energy/len(g))
        if best_delta == 0.0:
