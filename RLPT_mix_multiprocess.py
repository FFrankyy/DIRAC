...
def ExchangeMC(sg, g, nepochs, gid, fout):
    ...
    for epoch in range(nepochs):
        ...
        time_2 = time.time()
        print ('Lattice_dim: %d, test_scale: %d, Graph: %d, epoch: %d, time_cost: %.2f, result: %.8f'%(args.lattice_dim, args.test_scale, gid, epoch, time_2-time_1, -lowest_energy/len(g)))
...
def PTTest(lattice_dim, test_scale, nepochs, gid):
    ...
    fout = open('../TestData/PT_res/temp_data/Lattice_%dD_num_%d_RLPT_Mix_nepochs_%d_gid_%d.txt'%(lattice_dim, test_scale, nepochs, gid), 'w')
    ...
    best_energy, best_energy_improve, time_cost = ExchangeMC(sg, g, nepochs, gid, fout)
