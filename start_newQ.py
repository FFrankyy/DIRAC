...
def Evaluate():
    ...
    if args.lattice_dim == 2:
        ...
        model_file = './models/Lattice_2D_5_6_newQ/nrange_5_6_iter_617700.ckpt'
    ...
    if args.lattice_dim == 2:
        ...
        scales = [4,6,8,10,15,20]
    elif args.lattice_dim == 4:
        scales = [3]

    for num in scales:
        with open('../TestData/DQN_res/%s_num_%d_stepRatio_%.4f_MetaQ.txt'%(args.model_name, num, stepRatio), 'w') as fout:
...
def MultipleQCalc():
    ...
    # model_file = sg.findModel()
    sg.LoadModel(model_file)
    stepRatio = args.stepRatio

    # for num in [4,6,8,10,15,20]:
    for num in [6]:
        with open('../TestData/DQN_res/%s_num_%d_stepRatio_%.4f_MultiQ_1Inits.txt'%(args.model_name, num, stepRatio), 'w') as fout:
            result_list, result_improve_list, time_localImprove_list, time_list = [], [], [], []
            # read graph
            for i in tqdm(range(50)):
                g_file = '../TestData/Lattice/%dD/%d/%d.gml' % (args.lattice_dim, num, i)
                G = nx.read_gml(g_file)
                ...
                while not stopCondition:
                    ...
                    time_end = time.time()
                    print ('step:%d, energy:%.4f, time:%.4f'%(num_step, energy, time_end-time_start))
                    num_step += 1
                ...
                time2 = time.time()
                res_improve, _ = LocalImprove(g, init_states, res_best*len(g))
                time3 = time.time()

                result_list.append(res_best)
                result_improve_list.append(res_improve)
                time_localImprove_list.append(time3-time2)
                time_list.append(time3-time1)
                fout.write('Graph: %d, result: %.4f, improved result: %.4f, time_localImprove:%.2f, time:%.2f, Q_nums:%d\n' % (i, res_best, res_improve, time3-time2, time3-time1, num_step-1))
                fout.flush()
                print ('Graph: %d, result: %.4f, improved result: %.4f, time:%.2f\n' % (i, res_best, res_improve, time3-time1))
            fout.write('Result:%.4f+%.4f, Result_improve:%.4f+%.4f\n' % (np.mean(result_list), np.std(result_list), np.mean(result_improve_list), np.std(result_improve_list)))
            fout.write('Time_localImprove:%.2f+%.2f, time:%.2f+%.2f\n' % (np.mean(time_localImprove_list), np.std(time_localImprove_list), np.mean(time_list), np.std(time_list)))
            fout.flush()
