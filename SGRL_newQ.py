...
#################################################New code for graphDQN#####################################
    def BuildNet(self):
        ...
        # [embed_dim, embed_dim]
        p_node_conv = tf.Variable(tf.truncated_normal([3*args.embed_dim, args.embed_dim], stddev=0.01),tf.float32)
        ...
        e2npool = tf.sparse_tensor_dense_matmul(tf.cast(self.e2nsum_param, tf.float32), cur_edge_embed)
        ...
        while lv < args.max_bp_iter:
            ...
            #[node_cnt, 3*embed_dim]
            node_concat = tf.concat([n2npool, cur_node_embed, e2npool], axis=1)    #we used
            # [node_cnt, 3*embed_dim] * [3*embed_dim, embed_dim]
            node_linear = tf.matmul(node_concat, p_node_conv)
    ...
    def Train(self):
        ...
        f_out.write('loss, train_greedy, train_greedy_improve, valid_greedy, valid_greedy_improve\n')
        current_best, best_iter = 0.0, 0 # record the best result until now
        ...
        for iter in range(args.max_iteration):
            ...
            if iter % 300 == 0:
                ...
                valid_greedy, valid_greedy_improve = 0.0, 0.0
                ...
                f_out.write('%.4f, %.4f, %.4f, %.4f, %.4f\n'%(loss, train_greedy, train_greedy_improve, valid_greedy, valid_greedy_improve)) 
                f_out.flush()
                ...
                if iter > 5000 and current_best < valid_greedy:
                   current_best = valid_greedy
                   best_iter = iter
                print ('Current_best: %.4f, best_iter:%d'%(current_best, best_iter))
    ...
    def Evaluate(self, g, isNetworkx=1, step=1):
        ...
        while (not self.test_env.isTerminal()):
            ...
            for new_action in batchSol:
                if not self.test_env.isTerminal():
                    ...
                    break
        if best_len > 0:
            sol = sol_all[0:best_len]   # set of nodes to be flipped
            current_node_states = [initial[i] if i not in sol else -1*initial[i] for i in range(len(initial))]  # final node states
        else:
            best = init_energy
            current_node_states = initial
