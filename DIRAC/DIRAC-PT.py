...
def PTTest(lattice_dim, test_scale, nepochs, gid, hardness):
    sg = SGRL()
    ...
    fout = open('../TestData/DIRAC_PT_nepochs_%d_g_%d.txt'%(nepochs, gid), 'w')

    fout = open('../TestData/PT_res/Lattice_%dD_%d_DIRAC_PT_g_%d.txt'%(lattice_dim, test_scale, gid), 'w')

    print ('Dim:%d, Scale:%d, gid:%d'%(lattice_dim, test_scale, gid))
...
if __name__=="__main__":
    args = parsers()
    PTTest(args.lattice_dim, args.test_scale, args.numInits, args.gid, args.hardness)
