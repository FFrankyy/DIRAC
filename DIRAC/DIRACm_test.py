...
def DIRACm(sg, g, init_states):
	...
	return res_improve, numQ-1

def DIRACm_test(sg, lattice_dim, test_scale):
    fout = open('DIRACm_%dD_%d.txt'%(lattice_dim, test_scale),'w')
    for gid in range(50):
        ...
        res = DIRACm(sg, g, init_states)
        fout.write('DIRACm, Dim: %d, Size: %d, Graph: %d, res: %.8f\n'%(lattice_dim, test_scale, gid, res))
        fout.flush()
    fout.close()
...
if __name__=="__main__":
	...
	DIRACm_test(sg, args.lattice_dim, args.test_scale)
